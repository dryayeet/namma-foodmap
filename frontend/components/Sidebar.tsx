...
const HYPE_CHIP_ACTIVE: Record<HypeCategory, string> = {
  trending:   "bg-amber-400 text-slate-950 border-amber-400",
  overhyped:  "bg-red-400 text-slate-950 border-red-400",
  underrated: "bg-emerald-400 text-slate-950 border-emerald-400",
  neutral:    "bg-white text-slate-900 border-white",
};
...
export function Sidebar({
  ...
  return (
    <aside className="absolute top-0 left-0 z-[1001] h-screen w-[440px] max-w-[90vw] bg-slate-950/92 backdrop-blur-xl border-r border-white/[0.09] shadow-[0_10px_48px_rgba(0,0,0,0.55)] flex flex-col">
      <header className="px-5 pt-5 pb-4 border-b border-white/[0.06]">
        ...
        >
          <span className="inline-block w-2 h-2 rounded-full bg-amber-400 shadow-[0_0_10px_2px_rgba(250,204,21,0.55)]" />
      ...
      <div className="px-4 py-3 space-y-2.5 border-b border-white/[0.06]">
        <div className="relative">
          <input
            ...
            className="w-full text-sm pl-9 pr-8 py-2.5 rounded-lg bg-white/[0.04] border border-white/[0.08] text-slate-100 placeholder-slate-500 focus:bg-white/[0.06] focus:border-amber-400/40 focus:ring-2 focus:ring-amber-400/20 focus:outline-none transition"
          />
          <svg
          ...
          {searchDraft && (
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-200 text-xs w-5 h-5 flex items-center justify-center rounded-full hover:bg-white/10"
          ...
          {HYPES.map((h) => (
            <button
              ...
              className={clsx(
                ...
                filters.hype === h
                  ? HYPE_CHIP_ACTIVE[h]
                  : "bg-white/[0.03] text-slate-300 border-white/[0.08] hover:bg-white/[0.08] hover:text-slate-100"
              )}
            >
              {h}
            </button>
      ...
      <div className="px-4 py-2.5 border-t border-white/[0.06] text-[10px] text-slate-500 flex items-center justify-between">
        <span>{restaurants.length} results</span>
        <span className="text-slate-600">NammaNomNom v0.1</span>
      </div>
    </aside>
  );
}
...
function Stat({
  ...
  return (
    ...
    >
      ...
      <div className={clsx("font-bold text-sm mt-0.5", accent ? "text-amber-300" : "text-slate-100")}>
        {value}
      </div>
    </div>
  );
